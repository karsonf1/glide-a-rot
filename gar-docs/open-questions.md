...
- [ ] **Player ↔ corridor coupling** — ProcGen builds the treadmill window at the `RunCorridorOrigin` marker on glider deploy, but nothing ties the player's position/heading to it. Options: (a) teleport the player to `RunCorridorOrigin` on deploy, or (b) build the window at the player's deploy position instead of a fixed marker. Current stopgap: spawn placed at the origin facing +Z. Needs deciding before the first clean playtest.
